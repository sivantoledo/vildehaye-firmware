...
bool ubloxCommand (uint8_t* buffer, uint8_t class, uint8_t id, uint16_t len) { // message data should be in bytes 6 to 6+len-1 of the buffer
  uint8_t ck_a, ck_b;
  int i;
  buffer[0] = 0xB5;
  buffer[1] = 0x62;
  ...
  //System_printf("ubx(%d): ",len);
  //for (i=0; i<=6+len+1; i++) System_printf("%02x: ",buffer[i]);
  //System_printf("\n");
  ...
  i2cTransaction.writeCount   = 6+len+2;
  i2cTransaction.readBuf      = NULL;
  i2cTransaction.readCount    = 0;
  if (!I2C_transfer(i2c, &i2cTransaction)) {
    ubloxState = UBLOX_STATE_PORT_FAILURE;
    return false;
  } else
    return true;
}
...
#if 0
void sendRetry (uint8_t id1, uint8_t id2, uint8_t *data, uint16_t len) {
  acked = false;
  while (true) {
    ubloxCommand(id1, id2, data, len);
    if (acked)
      break;
    delay(100);
#if DEBUG
...
typedef struct __attribute__((packed)) {
  uint8_t  msgClass;
  uint8_t  msgId;
  uint8_t  rate; // on this port...
} CFG_MSG;

void enableMsg (uint8_t* buffer, uint8_t class, uint8_t msg, uint8_t rate) {
  ...
  ubloxCommand(buffer, 0x06, 0x01, sizeof(CFG_MSG));
  process(buffer); // process the ack;
}
...
void ubloxInit() {
  ...
  // switching did not work (on NEO-5Q)
  ...
  enableMsg(buffers[d.id], 0x01, 0x02, 4); // nav-posllh
  enableMsg(buffers[d.id], 0x01, 0x21, 4); // nav-timeutc
...
uint8_t bme280Initialized = 0;

void bme280Init() {
  uint8_t result = bme280SensorInit(1, 0); // the rate and config are not actually used...
  System_printf("bme280 Init result %d\n",result);
  bme280Start();
}

void bme280Sense() {
  uint32_t pressure = 98;
  ...
  bme280Start();
  System_printf("pressure is %d bytes_filled %d\n",pressure, bytes_filled);
}
